...
- Added M5Stack Core2 for AWS board target, with a new AXP192 PMU driver for its LCD/TF power rails, LCD reset line and backlight, FT6336U touch, BM8563 RTC and the 10-LED SK6812 bar. Its 8MB of PSRAM is enabled, which needs the peer comm UART moved off G17/G16 (the ESP32-D0WD PSRAM clock and chip select) to G19/G27, and a minimum silicon revision of 3 so the PSRAM cache workaround stops claiming 14KB of IRAM — this build will not boot on an older ESP32 revision. Driving the LED bar also needs the AXP192's EXTEN bit set, since the 5V boost it gates is what powers the M5GO base the LEDs sit on. Display, touch, SD, PSRAM, RTC, battery reporting, the setup wizard, the Apps store and the LED bar are all verified on hardware
- Fixed a view that switches to itself being silently dropped by the GUI router, which left the screen frozen on the old content. Affected every view that rebuilds itself after changing its own state, most visibly the setup wizard, where selecting a region lit the row up but never advanced a step
- Fixed `rgbmode off` after `rgbmode police` wedging the serial console while the display kept running: the police effect never tested the exit flag, so it was force-deleted while holding the RGB mutex and every later LED call blocked on it forever
- Fixed static `rgbmode <colour>` needing to be issued twice before the LEDs changed, by writing all pixels in one call instead of refreshing the whole strip once per LED
- Fixed a panic when any `rgbmode` command followed `rgbmode rainbow`: the rainbow task freed itself without clearing its task handle, so the next command called vTaskDelete on freed memory
