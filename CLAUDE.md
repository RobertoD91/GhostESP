...
- **Touching anything under `main/` means compiling one target per architecture, not just the board you care about.** Almost every board-specific branch in shared code (`esp_comm_manager.c`, `display_manager.c`) is selected by a runtime `strcmp(CONFIG_BUILD_CONFIG_TEMPLATE, …)`, so the compiler still has to emit the *whole* chain for every chip. A `GPIO_NUM_27` in an ESP32-only branch breaks every RISC-V build (the C3 stops at GPIO21), and a board build will never show it. `#ifdef CONFIG_IDF_TARGET_ESP32` around the branch is the fix. Build at minimum one Xtensa and one RISC-V target — e.g. `m5core2_aws` (esp32) and `ghostlink_p1_peer` (esp32c3). Chip architecture is not the only axis: `display_manager.c`'s touch and input paths are also cut by `GUI_LARGE_TOUCH_UI` (`include/gui/ui_capabilities.h`, P4 and RGB-LCD CrowPanels only), which wraps whole branches — `continue` statements included — that no other board compiles. Editing anything inside those needs a third build, e.g. `crowpanel_advanced_p4_rgb_800x480` (esp32p4). Grep the region you are touching for `#if` before deciding what "one per architecture" means.
- `. ~/esp-idf/export.sh` does **not** put the RISC-V toolchain on `PATH` even when it is installed, so C3/C5/C6 builds fail with "riscv32-esp-elf-gcc not found" on a machine that builds Xtensa targets fine. Install with `python ~/esp-idf/tools/idf_tools.py install riscv32-esp-elf` and prepend `~/.espressif/tools/riscv32-esp-elf/<version>/riscv32-esp-elf/bin` yourself.
...
**PSRAM on ESP32-classic collides with the peer UART defaults.** ESP-IDF's `D0WD_PSRAM_CLK_IO`/`D0WD_PSRAM_CS_IO` default to **17 and 16**, which are exactly `esp_comm_manager.c`'s `DEFAULT_TX_PIN`/`DEFAULT_RX_PIN`. Any ESP32-classic board that turns on `CONFIG_SPIRAM` must first move its comm UART, via the per-board `CONFIG_BUILD_CONFIG_TEMPLATE` branch in `esp_comm_manager_init()`. Pair `CONFIG_SPIRAM=y` with `CONFIG_SPIRAM_IGNORE_NOTFOUND=y` so a board whose PSRAM is wired to other pins still boots instead of aborting. Expect the link to then fail with `iram0_0_seg overflowed by ~14KB`: enabling PSRAM silently turns on `CONFIG_SPIRAM_CACHE_WORKAROUND`, which pulls libjmp/libmath/libio/libtime into IRAM. That workaround is only needed below silicon revision 3 (`depends on ESP32_REV_MIN_FULL < 300`), so on a board that ships a V3 part set `CONFIG_ESP32_REV_MIN_3=y` and it disappears — at the cost of refusing to boot on older ESP32 revisions, which is why it belongs in a board config and not in a shared default. Native SD apps (`plugin_loader.c`) refuse to run without PSRAM, which is what the "native app requires PSRAM" message in the Apps gallery means.

Turning PSRAM on for the first time on a board also arms a latent bug: GhostESP allocates several task stacks with `MALLOC_CAP_SPIRAM`, and while there is no PSRAM those allocations fail and quietly fall back to internal RAM. Once PSRAM exists they succeed, and FreeRTOS then aborts with `assert failed: xTaskCreateStaticPinnedToCore (xPortcheckValidStackMem(puxStackBuffer))` because a task stack may not live in external memory. `CONFIG_SPIRAM_ALLOW_STACK_EXTERNAL_MEMORY=y` is what the other PSRAM boards set; a `Failed to allocate … from PSRAM, falling back to internal` line in a boot log is the warning that this is coming.

Board pinouts come from the vendor's own pinmap, not from guesswork — for the Core2 for AWS that is <https://docs.m5stack.com/en/core/core2_for_aws>. On that board LCD/TF take G38/G23/G18/G5/G15/G4, I2C (touch, RTC, IMU) G21/G22 with touch INT on G39, audio G12/G0/G2/G34, and Port A G32/G33. Port B (G26/G36), Port C (G13/G14, the GPS UART here) and the 10 SK6812 LEDs on G25 live on the M5GO-Bottom For AWS base rather than the core unit — but that base ships as part of the product, so treat them as present and reachable. That leaves only **G19/G27** as free M-Bus GPIO, which is where the comm UART goes.
